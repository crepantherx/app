pipeline {
    ...
    stages {
		...
		steps {
		    	script {  
				sh "docker build -t crepantherx.jfrog.io/techmahindra-docker-dev-local/notes:v1.0.${env.BUILD_ID} -t crepantherx.jfrog.io/techmahindra-docker-dev-local/notes:latest -t crepantherx.jfrog.io/techmahindra-docker-dev-local/notes:${GIT_COMMIT} ."
		    	}
		}
	    }
	    stage('Test') {
		...
		steps {
				...
				docker.withRegistry('https://crepantherx.jfrog.io', 'jfrog') {
					...
					sh "docker push crepantherx.jfrog.io/techmahindra-docker-dev-local/notes:v1.0.${env.BUILD_ID}"
				}
		    	}
		}
	    }
